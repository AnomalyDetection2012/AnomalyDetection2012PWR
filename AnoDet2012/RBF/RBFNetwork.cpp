...
{
	this->kmeans = new KMeans(this->clustersNumber, this->kmeansMinSigmaSquare, this->kmeansMaxSigmaSquare, this->pointSize, this->kmeansIterations, this->kmeansStepImprovement, this->neighbours);
...
void RBFNetwork::setInput(vector<double>& input){
	...
	for(inputIt=input.begin(), inputLayerIt = this->inputLayer->begin();inputLayerIt!=this->inputLayer->end();++inputIt, ++inputLayerIt){
...
double RBFNetwork::getLastOutput(int index){
	return this->outputLayer[index]->lastOutputValue;
}
...
void RBFNetwork::learnNetwork(vector<vector<double> >& inputs, vector<vector<double> >& setpointsForEachOutputNeuron){
	...
	for(int generation=0; generation<this->generations; ++generation){
		...
		for(inputsIt=inputs.begin(), setpointsIt=setpointsForEachOutputNeuron.begin();inputsIt!=inputs.end();++inputsIt, ++setpointsIt){
			...
			for(setpointIt = setpointForEachOutputNeuron.begin(); setpointIt!=setpointForEachOutputNeuron.end();++setpointIt, ++currentOutputNeuronIndex){
				//cout<<currentOutputNeuronIndex<<endl;
...
double RBFNetwork::calculateQuantizationError(vector<double>& setpoints){
	...
	double qError=0.0;
	for(outputLayerIt=this->outputLayer.begin(), setpointsIt = setpoints.begin();outputLayerIt!=this->outputLayer.end();++outputLayerIt, ++setpointsIt){
...
vector<bool> RBFNetwork::test(vector<vector<double> > &set){
    ...
    for(setIt=set.begin();setIt!=set.end();++setIt){
        ...
        result.push_back(this->getLastOutput(0)>0.8);
    }
    return result;
}
